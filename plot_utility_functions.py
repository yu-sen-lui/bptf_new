...
def plot_all_components(factors, actors_N, actions_A, dates_T, title='', figdir=Path('plots')):
    figdir.mkdir(parents=True, exist_ok=True)
    print('Plotting...')
    for k in tqdm(range(factors[0].shape[1])):
        figname = f'k_{k}'
        if len(title) > 0:
            figname = title + '_' + figname
        filename = figdir.joinpath(figname)

        plot_component(Phi_s_V=factors[0][:, k].copy(),
                       Phi_r_V=factors[1][:, k].copy(), 
                       Phi_A=factors[2][:, k].copy(),
                       Theta_T=factors[3][:, k].copy(),
                       actors_N=actors_N, 
                       actions_A=actions_A,
                       dates_T=dates_T,
                       cdf_threshold=0.95, 
